...
from engines.utils.kfold_split import kfold_split
...
class DataManager:
    ...
    def __init__(self, configs, logger):
        ...
        self.kf_split = configs.kf_split
        ...
        if self.configs.use_pretrained_model:
            ...
            if self.dev_file is not None:
                ...
                if self.kf_split == 'False':
                    ...
                    for train_index, test_index in kfold.split(X, y, att_mask):
                        X_train = X[train_index]
                        y_train = y[train_index]
                        X_val = X[test_index]
                        y_val = y[test_index]
                        att_mask_train = att_mask[train_index]
                        att_mask_val = att_mask[test_index]
                    #X_train, X_val = kfold_split(X, 2)
                    #y_train, y_val = kfold_split(y, 2)
                    #att_mask_train, att_mask_val = kfold_split(att_mask, 2)
